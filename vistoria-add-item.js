...
// Botão contextual no final de cada local
// =========================================================
(function () {
  ...
  function criarBotoesLocais() {
    ...
    document.querySelectorAll('.sector').forEach(section => {
      ...
      botao.type = 'button';
      botao.className = 'btn btn-adicionar-item-local';
      botao.textContent = '+ Adicionar item';
      botao.dataset.local = local;
      botao.addEventListener('click', () => abrir(local));
      section.appendChild(botao);
    });
  }

  function criarInterface() {
    if (document.getElementById('add-item-overlay')) {
      criarBotoesLocais();
      return;
    }
    ...
    overlay.innerHTML = `
      <div class="add-item-dialog" role="dialog" aria-modal="true" aria-labelledby="add-item-titulo">
        <div class="add-item-cabecalho">
          <div><h3 id="add-item-titulo">Selecionar itens</h3><div id="add-item-local" class="add-item-local"></div></div>
    ...
    style.textContent = `
      .btn-adicionar-item-local{display:block;margin:16px auto 6px;min-height:42px;padding:9px 18px}
      ...
      @media(max-width:600px){.btn-adicionar-item-local{width:calc(100% - 16px)}.add-item-dialog{max-height:92vh}.add-item-rodape{display:grid;grid-template-columns:1fr 1fr}.add-item-rodape span{grid-column:1/-1}.add-item-rodape button{width:100%;padding:8px 6px;font-size:13px}}
    `;
  ...
  function renderLista() {
    ...
    if (!filtrados.length) { lista.innerHTML = '<div class="add-item-vazio">Nenhum item encontrado.</div>'; return; }
    lista.innerHTML = filtrados.map(item => `<label class="add-item-opcao"><input type="checkbox" data-id="${item.id}" ${selecionados.has(String(item.id)) ? 'checked' : ''}><span>${esc(item.descricao)}</span></label>`).join('');
  }
  ...
  async function adicionarSelecionados() {
    ...
    try {
      ...
      if (!novos.length) { alert('Os itens selecionados já existem neste local da vistoria.'); return; }
      ...
      await api('checklist_vistoria_itens', { method: 'POST', body: JSON.stringify(payload), prefer: 'return=minimal' });
      fechar();
      location.reload();
    } catch (erro) {
      ...
      botao.disabled = false; botao.textContent = 'Adicionar selecionados';
    }
  }
  ...
  function iniciar() {
    ...
    if (checklist) {
      const observer = new MutationObserver(() => criarBotoesLocais());
